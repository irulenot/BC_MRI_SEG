...
for i3, dataset in enumerate(datasets):
    ...
    for image_file, mask_file in tqdm(zip(images_files, masks_files)):
        ...
        if dataset == 'ISPY2' and seq.shape[1] == 1:  # ISPY2 has only some multiple angle labels
            new_channel = np.zeros_like(seq)
            seq = np.concatenate((seq, new_channel), axis=1)
        ...
        if dataset == 'ISPY2' and seq_masks.shape[1] == 1:
            new_channel = np.zeros_like(seq_masks)
            seq_masks = np.concatenate((seq_masks, new_channel), axis=1)
